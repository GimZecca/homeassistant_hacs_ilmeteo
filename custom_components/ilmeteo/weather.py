...
def _condition_from_symbol(id_simbolo, desc_breve: str | None = None) -> str:
    ...
    if desc_breve:
        ...
        for keyword, condition in _KEYWORD_FALLBACK:
            if keyword in d:
                _log_unmapped(key, desc_breve, condition)
                return condition

    _log_unmapped(key, desc_breve, "exceptional")
    return "exceptional"


# Evita di scrivere lo stesso warning ripetutamente nei log ad ogni
# aggiornamento (ogni 15 minuti, per più ore/giorni contemporaneamente).
_logged_unmapped: set[tuple[str, str]] = set()


def _log_unmapped(id_simbolo: str, desc_breve: str | None, fallback_used: str) -> None:
    combo = (id_simbolo, (desc_breve or "").strip().lower())
    if combo in _logged_unmapped:
        return
    _logged_unmapped.add(combo)
    if fallback_used == "exceptional":
        _LOGGER.warning(
            "3bMeteo: id_simbolo '%s' (desc_breve: '%s') non è mappato a nessuna "
            "condizione nota, mostrata icona 'exceptional'. Segnala questo valore "
            "per farlo aggiungere alla mappa.",
            id_simbolo, desc_breve,
        )
    else:
        _LOGGER.info(
            "3bMeteo: id_simbolo '%s' non è nella mappa principale, risolto "
            "tramite fallback testuale su '%s' -> '%s'.",
            id_simbolo, desc_breve, fallback_used,
        )
...
class Meteo3bWeather(CoordinatorEntity, WeatherEntity):
    ...
    def extra_state_attributes(self) -> dict:
        """Espone i dati grezzi usati per determinare la condizione attuale.

        Utile per diagnosticare rapidamente eventuali icone 'exceptional'
        senza dover controllare i log o rifare la chiamata API a mano.
        """
        r = self._ora_corrente
        return {
            "raw_id_simbolo": r.get("id_simbolo"),
            "raw_desc_breve": r.get("desc_breve"),
        }
